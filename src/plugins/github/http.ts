...
export interface GitHubHttpLimits {
  headerBytes: number;
  encodedBytes: number;
  decodedBytes: number;
  jsonNodes: number;
...
export interface GitHubJsonRequestOptions {
  ...
  timeoutMs: number;
  signal?: AbortSignal;
  limits?: Partial<GitHubHttpLimits>;
...
const DEFAULT_LIMITS: GitHubHttpLimits = {
  headerBytes: 32 * 1024,
...
export async function requestBoundedJson(
  ...
  const timer = setTimeout(
    () => controller.abort(new Error("github_request_timeout")),
    options.timeoutMs
  );

  try {
    if (options.signal?.aborted) throw new Error("github_request_aborted");
    let url = initialUrl;

    for (let redirects = 0; ; redirects += 1) {
      const remainingMs = deadline - Date.now();
      if (remainingMs <= 0) throw new Error("github_request_timeout");
      ...
      if (isRedirect(response.statusCode)) {
        ...
        if (redirects >= limits.redirects) throw new Error("github_redirect_limit");
        const location = headers.get("location");
        if (!location) throw new Error("github_redirect_missing_location");
...
function abortable<T>(promise: Promise<T>, signal: AbortSignal): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    promise.then(
      ...
      (error: unknown) => {
        signal.removeEventListener("abort", onAbort);
        reject(error instanceof Error ? error : new Error("github_request_failed"));
      }
    );
  });
}
...
function abortError(signal: AbortSignal): Error {
  return signal.reason instanceof Error ? signal.reason : new Error("github_request_aborted");
}

function validatedUrl(rawUrl: string): URL {
  const url = new URL(rawUrl);
  if (url.protocol !== "https:" && url.protocol !== "http:") {
    throw new Error("github_url_protocol");
  }
  if (url.username || url.password) throw new Error("github_url_credentials");
  return url;
}
...
function validatedRedirect(location: string, current: URL, allowedOrigin: string): URL {
  ...
  if (url.origin !== allowedOrigin) throw new Error("github_redirect_origin");
  return url;
}
...
function enforceHeaderLimit(headers: Dispatcher.ResponseData["headers"], maxBytes: number): void {
  ...
  for (const [name, value] of Object.entries(headers)) {
    ...
    if (bytes > maxBytes) throw new Error("github_response_headers_too_large");
  }
}
...
async function readJsonBody(
  ...
  if (contentLength !== undefined && contentLength > limits.encodedBytes) {
    body.destroy();
    throw new Error("github_response_encoded_too_large");
  }

  const chunks: Buffer[] = [];
  const encodedLimiter = byteLimitTransform(
    limits.encodedBytes,
    "github_response_encoded_too_large"
  );
  const decodedCollector = byteLimitTransform(
    limits.decodedBytes,
    "github_response_decoded_too_large",
    chunks
  );
...
function parseContentLength(value: string | null): number | undefined {
  ...
  if (!/^\d+$/.test(value)) throw new Error("github_content_length_invalid");
  const parsed = Number(value);
  if (!Number.isSafeInteger(parsed)) throw new Error("github_content_length_invalid");
  return parsed;
}
...
function contentDecoder(encodingHeader: string | null): Transform | undefined {
  ...
  throw new Error("github_content_encoding_unsupported");
}
...
function enforceJsonShape(value: unknown, limits: GitHubHttpLimits): void {
  const pending: { value: unknown; depth: number }[] = [{ value, depth: 0 }];
  let nodes = 0;
  while (pending.length > 0) {
    const current = pending.pop()!;
    nodes += 1;
    if (nodes > limits.jsonNodes) throw new Error("github_response_json_too_large");
    if (current.depth > limits.jsonDepth) throw new Error("github_response_json_too_deep");
