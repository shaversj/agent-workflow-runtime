...
import { OssRulesClient, type BenchmarkResponse } from "./client.js";
...
export function createReadinessReferenceTools(client = new OssRulesClient()): RegisteredTool[] {
  const authority = { target: "none", managedState: "read-write", network: "open" } as const;
  return [
    defineRegisteredTool({
      ...
      source: readinessPluginManifest.source,
      exposure: "hidden",
      readOnly: true,
      ...
      async execute(params: ReadCorpusEntryParams) {
        switch (params.kind) {
          ...
          case "project":
            return toolResult(
              "project",
              await client.readProject(required(params.repository, "ossrules_repository_required"))
            );
          case "skill":
            return toolResult(
              "skill",
