...
describe.skipIf(!image)("isolated coding preparation, approval and publication", () => {
  ...
  it("uses Pi editing in real workers, freezes checks, publishes exact files, reconciles a lost PR response, and remains inspectable", async () => {
    ...
    const transport: typeof fetch = async (url, init) => {
      ...
      if (endpoint === "git/commits") {
        const commit = parseCoding(
          Type.Object({
            author: Type.Object({ name: Type.String(), email: Type.String(), date: Type.String() }),
